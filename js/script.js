...
async function addToRecentlyPlayed(song) {
    try {
        ...
        // Step 4: Add to server (replacing localStorage)
        let recent = await getRecentSongs();
        recent = recent.filter(s => s.filename !== updatedSong.filename);
        recent.unshift(updatedSong);
        if (recent.length > 10) recent.pop();
        await saveRecentSongs(recent);
...
async function getRecentSongs() {
    try {
        const res = await fetch('/SpotifyResources/php/recently_played.php');
        const data = await res.json();
...
async function saveRecentSongs(songs) {
    try {
        const res = await fetch('/SpotifyResources/php/recently_played.php', {
            method: 'POST',
        ...
        console.error("Error saving recent songs:", err);
        return false;
    }
}
